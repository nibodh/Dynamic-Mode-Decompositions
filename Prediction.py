...
#     data_est = np.transpose(K_times_X,(1, 2, 0))
    ...
    Y_est = np.reshape(data_est,(nl,(m-1)*l))[:n,:]
    Y = np.reshape(data_f,(nl,(m-1)*l))[:n,:]
...
def N_step(data, K, poly_type, poly_order):  #basically applying powers of K
    ...
    data_est = np.empty([nl,m-1,l]) #initializing empty array b/c stupid python can't do it by itself
